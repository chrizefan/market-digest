...
Load all context per `SKILL-portfolio-manager.md` Pre-Flight section:
- `outputs/daily/{{DATE}}/macro.md` — regime classification
...
- `outputs/daily/{{DATE}}/DIGEST.md` (if completed)
...
Read `outputs/daily/{{DATE}}/opportunity-screen.md` (produced by `SKILL-opportunity-screener.md` in Phase 7B).

The screener output contains two pools:
...
For each ticker in the roster, follow `skills/SKILL-asset-analyst.md` completely.
Each analyst:
- Reads only Phase 1–5 session outputs (no new web searches)
- Is blinded to current portfolio weights
- Saves report to `outputs/daily/{{DATE}}/positions/{{TICKER}}.md`
...
Each defense is **appended** to the analyst's existing report in `positions/{{TICKER}}.md`:
...
| **Escalate** | Genuine 50/50 uncertainty, insufficient data to decide | Flag in `rebalance-decision.md` as OPEN QUESTION for user |
...
### Baseline Days (Sunday — SKILL-orchestrator.md Phase 7C)
...
### Delta Days (Mon–Sat — SKILL-daily-delta.md Phase 7C/7D, only when triggered)
...
Save the full deliberation transcript to: `outputs/daily/{{DATE}}/deliberation.md`
Template: `templates/deliberation-transcript.md`

After deliberation completes, hand off the resolved summary table to
`SKILL-portfolio-manager.md` **Phase B** (clean-slate construction) and **Phase C** (comparison).

---

## Quality Standards

1. **PM challenges must be specific** — not "is this right?" but "your bull case #2 assumes X, but macro.md says Y"
