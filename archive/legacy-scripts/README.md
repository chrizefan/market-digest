...
These Bash helpers printed prompts for a **markdown-in-`outputs/daily/`** workflow. The repo is now **DB-first**: canonical state lives in Supabase; use structured JSON + `scripts/materialize_snapshot.py` / `python3 scripts/run_db_first.py`.
...
| `archive.sh` | Move old `outputs/daily` to tarballs (destructive) |
