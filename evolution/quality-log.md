...
**Proposal**: Add fallback mode that reads from `data/agent-cache/daily/{{DATE}}/data/quotes.json` and `macro.json` pre-fetched files. These contain all needed price data.
...
2. `_load_prefetched_prices()` reads `data/agent-cache/daily/{latest}/data/quotes.json` as price source when yfinance unavailable
3. `deploy.yml` now runs `python scripts/update-tearsheet.py` before `npm run build` — docs always indexed from `data/agent-cache/` in CI
...
**Problem**: After pushing outputs, the web dashboard showed stale data. Documents didn't appear in the DigestTimeline. The `deploy.yml` workflow only ran `npm build` — it never regenerated `dashboard-data.json` from `data/agent-cache/`.
...
**Root cause**: The frontend is a static SPA reading a single `dashboard-data.json`. That JSON must be regenerated from `data/agent-cache/` at build time, not just committed from a local run.
