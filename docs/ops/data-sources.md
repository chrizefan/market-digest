...
> **Location:** this file was moved from `config/data-sources.md` so `config/` stays runtime-only. Stubs in `config/` point here.
...
By default the scripts write structured JSON + Markdown under a **local** `data/agent-cache/daily/YYYY-MM-DD/data/` tree (that path is **gitignored**; canonical artifacts still publish to **Supabase**).
